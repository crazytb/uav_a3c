...
The ATLAS framework compares two training paradigms: A3C with a shared global model and individual training with independent models. Both paradigms use identical network architectures and $T$-step rollout procedures.

\subsubsection{Network Architecture}

The agent employs a recurrent actor-critic network. At each decision epoch, the network receives the current observation $\mathbf{o} \in \mathbb{R}^{d}$ and the hidden state $\mathbf{h}$ carried from the previous epoch. Features are first extracted through an embedding layer with layer normalization:
\begin{equation}
