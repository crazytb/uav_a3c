...
\begin{equation}
\label{eq:S}
\mathcal{S} = \mathcal{S}_{obs} \times \mathcal{S}_{hid}
\end{equation}
where $\mathcal{S}_{obs} = \mathbf{S_{local}} \times \mathbf{S_{context}} \times \mathbf{S_{mec}}$ represents \textbf{observable states} directly measurable by the UAV agent, and $\mathcal{S}_{hid} = \mathbf{S_{cloud}} \times \mathbf{S_{channel}}$ represents \textbf{hidden states} that are not directly observable due to the asynchronous nature of cloud information ($\mathbf{S_{cloud}}$) or limitations in wireless channel sensing ($\mathbf{S_{channel}}$). The recurrent neural network architecture enables the agent to maintain internal memory and infer hidden state components through temporal patterns in observable measurements.
...
\end{equation}
where $\mathbf{C_l}$ represents a set of available computation units, $\mathbf{E_r}$ denotes a set of remaining epochs during one episode, $\mathbf{C_q}$ and $\mathbf{T_q}$ capture sets of queue computation units and processing times, and $\mathbf{I_l}, \mathbf{I_o}$ provide binary success indicators for local and offload operations. Above-mentioned values are normalized within $[0, 1]$ to ensure stable neural network training and effective learning across different operational scales.
...
\begin{equation}
\mathbf{S_{context}} = \mathbf{V} \times \mathbf{C}
\end{equation}
...
where $\mathbf{V}$ represents normalized velocity context, and $\mathbf{C}$ indicates normalized local computation capacity context. We set the minimum and the maximum velocities to \SI{30}{km/h} and \SI{100}{km/h}~\cite{3GPP_TS_22.125}, respectively, and the computation capacity ranges from 0 to 200 units. Both $\mathbf{V}$ and $\mathbf{C}$ are normalized to $[0, 1]$ according to their respective marginal values.
...
\begin{equation}
\mathbf{S_{{mec}}} = \mathbf{C_m} \times \mathbf{T_m}
\end{equation}
where $\mathbf{C_m} = \prod_{k \in K} \mathbf{C_{m,k}}$ represents ongoing computation units for each task $k \in K$, and $\mathbf{T_m} = \prod_{k \in K} \mathbf{T_{m,k}}$ denotes Corresponding processing times for each task $k \in K$. UAV agents can observe MEC state through acknowledgement messages and status updates from edge infrastructure.
...
\begin{equation}
    s = \{c_l, e_r, c_q, t_q, i_l, i_o, v, c, \mathbf{c}_m, \mathbf{t}_m, \mathbf{c}_c, \mathbf{t}_c, q\}
\end{equation}
and
\begin{equation}
    s' = \{c_l', e_r', c_q', t_q', i_l', i_o', v', c', \mathbf{c}_m', \mathbf{t}_m', \mathbf{c}_c', \mathbf{t}_c', q'\}
...
The complete transition function can be decomposed into \textbf{observable} and \textbf{hidden} components, as mentioned in~\eqref{eq:S}:
\begin{equation}
    \Pr[s' | s, a] = \Pr[s_{obs}' | s, a] \times \Pr[s_{hid}' | s],
\end{equation}
where $s_{obs}' \in \mathcal{S}_{obs}$ and $s_{hid}' \in \mathcal{S}_{hid}$, respectively, making sure that the dynamics of exogenous states are independent from the agent's action. The endogenous components evolve according to controllable system dynamics influenced by the agent's actions, while the exogenous components evolve independently, modeling external task arrivals and environmental conditions. This decomposition explicitly separates controllable resource allocation from uncontrollable workload variations, enabling the agent to learn robust policies under stochastic task arrivals.

where $s_{obs}' \in \mathcal{S}_{obs}$ represents the observable state components directly accessible to UAV agents, and external factors, such as channel quality $Q$ and cloud information, $C_c$ and $T_c$ influence state transitions without being part of the state representation. Observable state components include both action-dependent transitions and action-independent transitions. This formulation explicitly captures the partial observability arising from hidden wireless channel conditions and asynchronous multi-agent cloud resource competition, enabling agents to learn robust policies through implicit belief state inference via recurrent neural networks.


\subsubsection{Observable State Transitions}

Observable state transitions are governed by the agent's action selection and are fully determined by the current observable state. These include resource consumption ($c_l$ updates), queue management ($\mathbf{c}_m, \mathbf{t}_m, \mathbf{c}_c, \mathbf{t}_c$ modifications), and action outcome indicators ($i_l, i_o$). In contrast to hidden state transitions, observable components evolve deterministically given the action and current resource availability.
...
% \textbf{Local Computation Units ($c_l$):} [설명...]

% \textbf{Remaining Epochs ($e_r$):} [설명...]

% \textbf{Success Indicators ($i_l, i_o$):} [설명...]

% \textbf{MEC Server State ($\mathbf{c}_m, \mathbf{t}_m$):} [설명...]

\subsubsection{Exogenous State Transitions}

The exogenous components evolve independently of the agent's actions and model external task arrivals and environmental variations:

\begin{equation}
\Pr[s_{\text{exo}}' | s] = \Pr[c_q'] \times \Pr[t_q'] \times \Pr[v'] \times \Pr[c']
\end{equation}

\textbf{Queue Task Arrivals ($c_q, t_q$):} New tasks arrive at each time step with computational requirements and processing times sampled from uniform distributions:
\begin{equation}
c_q' \sim \text{Uniform}(1, 200), \quad t_q' \sim \text{Uniform}(1, 50)
\end{equation}
This stochastic arrival process models the unpredictable workload in real-world UAV applications.

\textbf{Environmental Context ($v, c$):} Velocity and computation capacity contexts remain constant within an episode, representing the environmental configuration:
\begin{equation}
v' = v, \quad c' = c
\end{equation}
