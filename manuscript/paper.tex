...
% FROM HERE
The exogenous components evolve independently of the agent's actions, comprising stochastic task arrivals, wireless channel dynamics, and fixed environmental context:
\begin{equation}
P[s_{\text{exo}}' | s] = P[c_q'] \times P[t_q'] \times P[Q' | Q],
\end{equation}
with the environmental context satisfying $v' = v$ and $c' = c$ throughout each episode.
...
\end{equation}
where $f_0 = 5.9\,\text{GHz}$ is the carrier frequency (IEEE 802.11bd) and $c_{\text{light}}$ is the speed of light.
...
\begin{equation}
P[s' | s, a] = P[s_{\text{endo}}' | s, a] \times P[s_{\text{exo}}' | s]
\end{equation}
