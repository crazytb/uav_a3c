...
%     \begin{equation}
%     \Pr[\mathbf{C}_c', \mathbf{T}_c' | \mathbf{C}_c, \mathbf{T}_c, a_i] \quad \text{(not well-defined)}
%     \end{equation}
...
% The discrete action formulation enables straightforward policy optimization through categorical probability distributions, allowing the neural network to output action probabilities $\pi(a|s)$ for effective exploration and exploitation during training.
...
% % \begin{equation}
% % \mathcal{P}(s'|s, a) = P(s' = (c', q', e', v')|s = (c, q, e, v), a)
% % \end{equation}
...
The reward function $\mathcal{R}: \mathcal{S} \times \mathcal{A} \rightarrow \mathbb{R}$ provides immediate feedback to guide the UAV agent's learning process by quantifying the desirability of taking action $a$ in state $s$. The ATLAS framework designs a multi-objective reward function that balances energy efficiency, processing latency, and task completion success.
...
\begin{aligned}
&\mathcal{R}(s, a) \\
& = -\alpha \cdot C_{energy}(s, a) \\
& - \beta \cdot C_{latency}(s, a) \\
& - \gamma \cdot C_{failure}(s, a) \\
& + \delta \cdot B_{completion}(s, a)
\end{aligned}
\end{equation}
...
\begin{equation}
C_{energy}(s, a) = \begin{cases}
E_{local} \cdot q_{units} & \text{if } a = 0 \\
E_{transmission} \cdot q_{units} & \text{if } a = 1 \\
0 & \text{if } a = 2
\end{cases}
\end{equation}
...
\begin{equation}
C_{latency}(s, a) = \begin{cases}
T_{local}(q_{units}, c_{avail}) & \text{if } a = 0 \\
T_{offload}(q_{units}, e_{mec\_times}) + T_{transmission} & \text{if } a = 1 \\
0 & \text{if } a = 2
\end{cases}
\end{equation}
...
\begin{equation}
C_{failure}(s, a) = \begin{cases}
P_{fail\_local} \cdot \rho_{failure} & \text{if } a = 0 \\
P_{fail\_offload} \cdot \rho_{failure} & \text{if } a = 1 \\
\rho_{discard} & \text{if } a = 2
\end{cases}
\end{equation}
...
\begin{equation}
B_{completion}(s, a) = \begin{cases}
(1 - P_{fail\_local}) \cdot \eta_{success} & \text{if } a = 0 \\
(1 - P_{fail\_offload}) \cdot \eta_{success} & \text{if } a = 1 \\
0 & \text{if } a = 2
\end{cases}
\end{equation}
    ...
    \For{each worker $w \in \{1, \ldots, W\}$ \textbf{in parallel}}
        ...
        \For{episode $e = 1$ to $E_{\max}$}
            ...
            \While{episode not terminated}
                ...
                \For{$t = 0$ to $T-1$}
                    \State Compute $\pi(\cdot | s, h_t; \theta_w)$ and $V(s, h_t; \theta_w)$
                    \State Sample $a \sim \pi(\cdot | s, h_t; \theta_w)$
                    \State Execute $a$, observe $r_t$ and $s'$
                    \State Update $h'$ via GRU; if done, $h' \gets \mathbf{0}$
                \EndFor
                \State \textbf{// Compute returns and advantages}
                \State Bootstrap $R \gets V(s, h_T; \theta_w)$ if not done, else $R \gets 0$
                \For{$t = T-1$ down to $0$}
                    \State $R \gets r_t + \gamma R$; $A_t \gets R - V(s, h_t; \theta_w)$
                \EndFor
                \State \textbf{// Compute loss}
                \State $\mathcal{L} \gets -\sum_t \log \pi(a | s, h_t; \theta_w) A_t + \beta_v \sum_t (R_t - V_t)^2 - \beta_h \mathcal{H}(\pi)$
