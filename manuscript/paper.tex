...
\begin{equation}
\label{eq:S}
\mathcal{S} = \mathcal{S}_{endo} \times \mathcal{S}_{exo}
\end{equation}
where $\mathcal{S}_{endo} = \mathbf{S_{local}} \times \mathbf{S_{context}} \times \mathbf{S_{mec}}$ represents \textbf{endogenous states} whose transitions are agent-internal (controllable dynamics), and $\mathcal{S}_{exo} = \mathbf{S_{cloud}} \times \mathbf{S_{channel}}$ represents \textbf{exogenous states} whose transitions are uncontrollable due to the asynchronous nature for cloud information ($\mathbf{S_{cloud}}$) or is hidden to each UAV agent for wireless channel state ($\mathbf{S_{channel}}$).
...
\end{equation}
making sure that the dynamics of exogenous states are independent from the agent's action.

The endogenous components ($c_l, e_r, i_l, i_o, \mathbf{c}_m, \mathbf{t}_m$) evolve according to controllable system dynamics influenced by the agent's actions, while the exogenous components ($c_q, t_q, v, c$) evolve independently, modeling external task arrivals and environmental conditions. This decomposition explicitly separates controllable resource allocation from uncontrollable workload variations, enabling the agent to learn robust policies under stochastic task arrivals.
