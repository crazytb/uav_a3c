...
\begin{equation}
\mathcal{S} = {S}_{loc} \times {S}_{con} \times {S}_{ser} \times {S}_{cha} 
\end{equation}

where each component represents distinct aspects of the system state:

\textbf{Local Device State ($S_{loc}$):} This component characterizes the UAV's local computational resources, task queue, and processing feedback:
\begin{equation}
S_{\text{local}} = C_l \times E_r \times C_q \times T_q \times I_l \times I_o
\end{equation}
...
\textbf{Environmental Context ($S_{con}$):} This encompasses dynamic environmental conditions:
\begin{equation}
S_{\text{context}} = V \times C
\end{equation}
where $V$ represents normalized velocity context, and $C$ indicates normalized computation capacity context. The minimum and the maximum velocities are set to 30 and 100, respectively,
        self._COMP_MAX = 120.0  # your randint upper bound

\textbf{Server State ($S_{ser}$):} This component models the MEC and cloud infrastructure availability:
\begin{equation}
S_{\text{server}} = S_{\text{mec}} \times S_{\text{cloud}}
\end{equation}
where $S_{\text{mec}} = C_m^K \times T_m^K$ and $S_{\text{cloud}} = C_c^K \times T_c^K$, with $C_m, C_c \in [0, 1]^K$ representing normalized computation units and $T_m, T_c \in [0, 1]^K$ denoting processing times for $K$ tasks in the server queues.

\textbf{Channel State ($S_{cha}$):} This captures the wireless communication quality:
\begin{equation}
S_{\text{channel}} = Q
\end{equation}
where $Q \in \{1, 2, \ldots, Q_{\max}\}$ represents discrete channel quality levels.

All continuous values are normalized to the inverval $[0, 1]$ to ensure stable neural network training, while binary indicators take values in $\{0, 1\}$ to encode operation success

The complete state vector at time $t$ is represented as:
\begin{equation}
s_t = (C_l^{(t)}, E_r^{(t)}, C_q^{(t)}, T_q^{(t)}, I_l^{(t)}, I_o^{(t)}, Q^{(t)}, C_m^{(t)}, T_m^{(t)}, C_c^{(t)}, T_c^{(t)}, V^{(t)}, C^{(t)})
\end{equation}
