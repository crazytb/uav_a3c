...
&\times P(c_q') \\
&\times P(t_q') \\
&\times P(i_l', i_o' | c_l, c_q, c_m) \\
&\times P(c_m' | c_m, c_q, a) \\
&\times P(t_m' | t_m, t_q, a),
\end{split}
\end{equation}
...
The offload feasibility additionally depends on hidden state components $Q \in \mathcal{Z}$ and $\mathbf{c}_c \in \mathcal{Z}$.
...
\textbf{MEC Server State ($\mathbf{c}_m, \mathbf{t}_m$):} The MEC queue undergoes a three-phase update per decision epoch: enqueue, completion removal, and processing-time decrement.
