...
\end{equation}
where $s_{obs}' \in \mathcal{S}_{obs}$ and $s_{hid}' \in \mathcal{S}_{hid}$, respectively,
making sure that the dynamics of exogenous states are independent from the agent's action. The endogenous components evolve according to controllable system dynamics influenced by the agent's actions, while the exogenous components evolve independently, modeling external task arrivals and environmental conditions. This decomposition explicitly separates controllable resource allocation from uncontrollable workload variations, enabling the agent to learn robust policies under stochastic task arrivals.
