...
The complete transition function can be decomposed into \textbf{endogenous} (agent-internal) and \textbf{exogenous} (agent-external or environment-driven) components, as mentioned in~\eqref{eq:S}:
\begin{equation}
    \Pr[s' | s, a] = \Pr[s_{endo}' | s, a] \times \Pr[s_{exo}' | s],
\end{equation}
where $s_{endo}' \in \mathcal{S}_{endo}$ and $s_{exo}' \in \mathcal{S}_{exo}$, respectively,
